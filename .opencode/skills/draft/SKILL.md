...
Before writing artifacts, create `outputs/.plans` and `papers`.
...
- Use the `writer` agent when the draft should be produced from already-collected notes, then use the `verifier` agent to add inline citations and verify sources. If the task tool or either agent is unavailable or fails, continue lead-owned and mark verification as `BLOCKED` or `PASS WITH NOTES`.
...
- Save exactly one draft to `papers/<slug>.md`. If source material or verification is unavailable, still write a blocked or partial draft artifact with the limitation clearly marked.
- End with a `Sources` appendix with direct URLs for all primary references.
- Before responding, verify on disk that `papers/<slug>.md` exists.
