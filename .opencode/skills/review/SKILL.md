...
Review the user's AI research artifact.

Derive a short slug from the artifact name: lowercase, hyphens, no filler words, at most 5 words. Use this slug for all files in this run.
...
2. Write `outputs/.plans/<slug>-review-plan.md` with the artifact identifier and source type, review criteria, and verification checks needed for claims, figures, reported metrics, data/code availability, and linked artifacts.
3. Continue immediately. Do not end after planning.
4. Inspect the artifact. For local files, read or parse the file directly. For PDFs, use available PDF/document parsing tools; if parsing fails, use available fallback extraction, record the failure, and still produce a blocked or partial review artifact. For arXiv IDs or URLs, fetch the paper/source directly and record the URL. Inspect linked code, datasets, supplemental material, or citations when they are reachable and materially affect the review.
...
6. Use the `researcher` and `reviewer` agents only if the task tool is available and the artifact is large enough to benefit from delegation. If agents are unavailable, fail, or would only add overhead, do the lead-owned review directly. Never merely say an agent was spawned; either call the tool or continue yourself.
7. Write exactly one final review artifact to `outputs/<slug>-review.md` with Summary Assessment, Strengths, Critical Issues, Major Issues, Minor Issues, Reproducibility and Verification, Inline Annotations tied to sections/claims/figures/tables where possible, Recommendation, and Sources.
